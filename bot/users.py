...
class Users:
    ...
    def get_id(self, uid) -> int:
        if type(uid) == Message:
            ...
            return (
                uid.from_user.id
                if self.do_override(uid) or not uid.message
                else uid.message.chat.id
            )
    ...
    def set(self, uid, item, value):
        ...
        if self.chat.get(uid) and i:
            if i.is_valid(value):
                ...
                raise ValueError(
                    f"Type mismatch: {type(self.chat[uid][item])} and {type(value)} ({value})"
                )
        return False
    ...
    def can_send_to(self, uid: int) -> bool:
        if uid:
            try:
                ...
                log.info(f"[{uid}] is an invalid id. Forgetting it.")
                self.forget(uid)
