...
class Handlers:
    ...
    def run(self, LANG, bot, m):  # repeat command
        if m.reply_to_message:
            ...
            elif self.cfg.is_admin(m) or (
                m.from_user and m.from_user.id == m.reply_to_message.from_user.id
            ):
    ...
    def message(self, bot, m):
        ...
        if (
            ...
            try:
                ...
                sender = (
                    (
                        f"{m.from_user.mention()}, <code>{m.from_user.id}</code>"
                        + (
                            f" at <code>{m.chat.id}</code>"
