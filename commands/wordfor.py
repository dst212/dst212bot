...
class CmdWordFor(BaseCommand):
    ...
    def function(self, LANG, definition: str, limit: int = 1):
        res = requests.get(
            API + "related?term=" + urllib.parse.quote(definition, safe="")
        )
        if res.status_code != 200:
            return LANG("WEBSITE_UNAVAILABLE"), LANG(
                "IS_NOT_AVAILABLE_AT_THIS_TIME"
            ).format(CREDITS_WEBSITE)
        words = res.json()
        if len(words) == 0:
            ...
            return [
                InlineQueryResultArticle(
                    ...
                    input_message_content=InputTextMessageContent(
                        LANG("PROVIDE_SEARCH_QUERY")
                    ),
                    description=LANG("PROVIDE_SEARCH_QUERY"),
                )
            ]
        text = q.text[i+1:]
        results = self.function(LANG, text, limit=7)
        return [
            InlineQueryResultArticle(
                id=r["word"],
                title=r["word"],
