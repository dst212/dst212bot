    ...
    if key:
        ...
        if not os.path.exists(file_path):
            ...
            with open(file_path, "w") as f:
                api = {}
                for item in json.loads(requests.get(API + key + "?limit=9999").text)[
                    "results"
                ]:
    ...
    if not ret:
        ...
        with open(file_path, "w") as f:
            ...
            for k, v in json.loads(
                requests.get(API + category + "/" + item).text
            ).items():
                if k == "names":
                    ret[k] = {}
                    for i in v:
                    ...
                    for i in v:
                        ret[k][i["move"]["name"]] = {
                            ...
                            "method": i["version_group_details"][-1][
                                "move_learn_method"
                            ]["name"],
                        }
                elif k == "abilities":
                    ...
                    for i in v:
                        ret[k][
                            (i["version"] if "version" in i else i["generation"])[
                                "name"
                            ]
                        ] = i["game_index"]
                elif k == "held_items":
                    ...
                    for i in v:
                        ...
                        for j in i["version_details"]:
                            ret[k][i["item"]["name"]][j["version"]["name"]] = j[
                                "rarity"
                            ]
                else:
                    ret[k] = v
