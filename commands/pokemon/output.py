...
def print_move(res) -> str:
    text = (
        ...
        f"{f"<b>Power</b>: {res["power"]}\n" if res.get("power") else ""}"
        "<b>Accuracy</b>: "
        f"{f"{res["accuracy"]}%""" if res["accuracy"] else "can't miss"}\n"
        f"<b>PP</b>: {res["pp"]}\n"
        f"{"" if res["priority"] == 0 else "<b>Priority</b>: {0:+}\n".format(res["priority"])}"
    )
    # meta data
    if res["meta"]:
    ...
    if res["effect_entries"]:
        if res["effect_chance"] is not None:
            text += (
                res["effect_entries"]["en"].replace(
                    "$effect_chance", str(res["effect_chance"])
                )
                + "\n"
            )
        else:
    ...
    text = (
        ...
        f"<b>Ability</b>: {
            ", ".join(
                ...
                + (" (hidden)" if v["is_hidden"] else "") for k, v in res["abilities"].items())}\n"
    )
    text += "<b>Stats</b>:\n"
    for k, v in res["stats"].items():
        text += f"- {k.replace("special-", "Sp. ").capitalize()}: <code>{v["base_stat"]}</code>"
        if v["effort"]:
...
def print_ability(res) -> str:
    ...
    if res.get("effect_entries"):
        text += f"\n<b>Effect</b>: {res["effect_entries"]["en"]}\n"
    if res.get("flavor_text_entries"):
