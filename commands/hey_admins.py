...
class CmdHey(BaseCommand):
    ...
    def forward(self, m):
        ...
        self.cfg.forward(
            info_msg, forward=[m.reply_to_message or m], exclude=[m.chat.id]
        )
    ...
    def parse(self, bot, m) -> bool:
        ...
        if not (m.text or m.caption or "").startswith("/"):
            ...
            if mdata and self.cfg.is_helper(m) and self.cfg.is_in(m.chat.id, "support"):
                bot.copy_message(
                    mdata[0], m.chat.id, m.id, reply_to_message_id=mdata[1]
                )
                for i in self.cfg.get_support_chats():
                    if i != m.chat.id:
                        try:
                            bot.send_message(
                                i,
                                format_user(m.from_user or m.sender_chat or m.chat)
                                + f" in reply to <code>#{mdata[0]}</code><code>#{mdata[1]}#</code>",
                            )
        ...
        if sender_is_admin(c.callback):
            if c.args[1] == "forward":
                c.callback.edit_message_text(
                    self.enable(LANG, c.callback.message.chat, c.callback.from_user)
                )
            elif c.args[1] == "cancel":
                c.callback.edit_message_text(
                    LANG("CHAT_FORWARD_NOT_ENABLED")
                    if not self.usr.do_forward(c.callback.message.chat.id)
                    else self.disable(
                        LANG, c.callback.message.chat, c.callback.from_user
                    )
                )
        else:
