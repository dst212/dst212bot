...
class CmdCounter(BaseCommand):
    ...
    def get(self, m, counter):
        ...
        m.reply(
            self.usr.lang(m, "COUNTER_IS").format(
                html.escape(data["display"]), data["value"]
            )
            if data
            else self.usr.lang(m, "COUNTER_DOESNT_EXIST").format(counter)
        )
                ...
                for k, v in group.items():
                    ...
                    for word in v["triggers"]:  # log.info(f"FOR {word}")
                        if word in text:
                            ...
                            m.reply(
                                self.usr.lang(m, "COUNTER_SET").format(
    ...
    def rem(self, m, counter):
        ...
        if not data:
            ...
            if group and group.get(counter):
                del group[counter]
            m.reply(
                self.usr.lang(m, "COUNTER_DELETED").format(
                    html.escape(data["display"]), data["value"]
                )
            )
    ...
    def display(self, m, counter, display):
        ...
        if not data:
            ...
            self.update_group(str(m.chat.id), counter, data)
            m.reply(
                self.usr.lang(m, "COUNTER_DISPLAY_SET").format(
                    counter, html.escape(display)
                )
            )
    ...
    def auto_add(self, m, counter, word):
        ...
        if not data:
            ...
            group = self.groups.get(str(m.chat.id))
            if not group:
                group = self.groups[str(m.chat.id)] = {}
            group[counter] = data
            m.reply(
                self.usr.lang(m, "COUNTER_AUTO_HAS").format(
                    html.escape(data["display"]),
                    html.escape(
                        ", ".join(data["triggers"])
                        or self.usr.lang(m, "COUNTER_NO_TRIGGERS")
                    ),
                )
            )
        ...
        if not data:
            ...
            try:
                ...
                m.reply(
                    self.usr.lang(m, "COUNTER_AUTO_HAS").format(
                        html.escape(data["display"]),
                        html.escape(", ".join(data["triggers"])),
                    )
                )
            except Exception:
                m.reply(
                    self.usr.lang(m, "COUNTER_WORD_NOT_FOUND").format(
                        word,
                        html.escape(
                            ", ".join(data["triggers"])
                            or self.usr.lang(m, "COUNTER_NO_TRIGGERS")
