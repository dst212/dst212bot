...
class CmdCounter(Command):
	...
	def load(self, chat, counter) -> dict:
		...
		if self.is_valid_name(counter) and os.path.exists(cd + counter):
			...
			if ret["triggers"]:
				...
				if not self.groups.get(chat):
					self.groups[chat] = {}
				self.groups[chat][counter] = ret
		return ret
	...
	def load_all(self):
		...
		for chat in os.listdir(self.base_dir):
			for counter in os.listdir(self.base_dir + chat):
				self.load(chat, counter)
		self.mutex.release()

	def get(self, m, counter):
		data = self.load(str(m.chat.id), counter)
		m.reply(self.usr.lang(m, 'COUNTER_IS').format(html.escape(data["display"]), data["value"]) if data else self.usr.lang(m, 'COUNTER_DOESNT_EXIST').format(counter))
	...
	def parse_message(self, m):
		...
		if text:
			text = text.lower()
			self.mutex.acquire()
			group = self.groups.get(str(m.chat.id))
			if group: # log.info("COUNTER group exists")
				for k, v in group.items():
					...
					for word in v["triggers"]: # log.info(f"FOR {word}")
						if word in text:
							saved = False
							self.groups[str(m.chat.id)][k]["value"] += v["step"]
							m.reply(self.usr.lang(m, 'COUNTER_SET').format(html.escape(v["display"]), v["value"]))
					if not saved:
						self.save(str(m.chat.id), k, v)
			self.mutex.release()
		
	def new(self, m, counter):
		if os.path.exists(f"{self.base_dir}{str(m.chat.id)}/{counter}"):
			m.reply(self.usr.lang(m, 'COUNTER_ALREADY_EXISTS'))
			...
			m.reply(self.usr.lang(m, 'COUNTER_COULDNT_CREATE'))
		else:
			m.reply(self.usr.lang(m, 'COUNTER_CREATED'))

	def rem(self, m, counter):
		data = self.load(str(m.chat.id), counter)
		if not data:
			m.reply(self.usr.lang(m, 'COUNTER_DOESNT_EXIST').format(counter))
		elif not m.from_user or m.from_user.id != data["owner"]:
			m.reply(self.usr.lang(m, 'COUNTER_YOU_ARENT_THE_OWNER'))
		else:
			os.remove(f'{self.base_dir}{str(m.chat.id)}/{counter}')
			group = self.groups.get(str(m.chat.id))
			if group and group.get(counter):
				del group[counter]
			m.reply(self.usr.lang(m, 'COUNTER_DELETED').format(html.escape(data["display"]), data["value"]))
	...
	def set(self, m, counter, value=None, add=False):
		...
		if not data:
			...
			m.reply(self.usr.lang(m, 'COUNTER_YOU_ARENT_AN_EDITOR'))
		else:
			if add:
			...
			m.reply(self.usr.lang(m, 'COUNTER_SET').format(html.escape(data["display"]), data["value"]))
	...
	def ren(self, m, counter, newname):
		...
		if not data:
			...
			path = f'{self.base_dir}+{str(m.chat.id)}/'
			os.rename(path+counter, path+newname)
			group = self.groups.get(str(m.chat.id))
			if group and group.get(counter):
				group[newname] = group[counter]
				del group[counter]["value"]
			m.reply(self.usr.lang(m, 'COUNTER_RENAMED_FROM').format(counter, newname))
	...
	def display(self, m, counter, display):
		...
		if not data:
			...
			self.update_group(str(m.chat.id), counter, data)
			m.reply(self.usr.lang(m, 'COUNTER_DISPLAY_SET').format(counter, html.escape(display)))
	...
	def auto_add(self, m, counter, word):
		...
		if not data:
			...
			self.save(str(m.chat.id), counter, data)
			group = self.groups.get(str(m.chat.id))
			if not group: group = self.groups[str(m.chat.id)] = {}
			group[counter] = data
			m.reply(self.usr.lang(m, 'COUNTER_AUTO_HAS').format(html.escape(data["display"]), html.escape(", ".join(data["triggers"]) or self.usr.lang(m, 'COUNTER_NO_TRIGGERS'))))
		...
		if not data:
			...
			try:
				...
				if len(data["triggers"]) == 0:
					del self.groups[str(m.chat.id)][counter]
				else:
					self.update_group(str(m.chat.id), counter, data)
				m.reply(self.usr.lang(m, 'COUNTER_AUTO_HAS').format(html.escape(data["display"]), html.escape(", ".join(data["triggers"]))))
			except:
				m.reply(self.usr.lang(m, 'COUNTER_WORD_NOT_FOUND').format(word, html.escape(", ".join(data["triggers"]) or self.usr.lang(m, 'COUNTER_NO_TRIGGERS'))))
