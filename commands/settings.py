...
class CmdSettings(BaseCommand):
    ...
    def button(self, LANG, m, callback, option_name: str):
        return InlineKeyboardButton(
            f"""{LANG('SETTINGS_' + option_name.upper())}: {self.usr.get(m.chat.id, option_name)}""",
            callback + option_name,
        )

    def bool_button(self, LANG, m, callback, option_name: str):
        return InlineKeyboardButton(
            f"""{"✅" if self.usr.get(m.chat.id, option_name) else "❌"} {LANG('SETTINGS_' + option_name.upper())}""",
            callback + option_name,
        )
    ...
    def list_buttons(
        ...
        return InlineKeyboardMarkup(
            [[InlineKeyboardButton(LANG("BACK"), back_button or self.name)]]
            + [
                [  # first_row + [[
    ...
    def gen_markup(self, LANG, m):
        ...
        buttons = [
            ...
            [
                InlineKeyboardButton(
                    f"""🌐 Language: {langs.flag(lang)}{langs.formal_name(lang)}""",
                    callback + "lang",
                )
            ],
        ]
            ...
            # settings in the start message
                ...
                if value == "set":
                    ...
                    if value is not None:
                        ...
                        LANG = langs.Lang(
                            self.usr.lang_code(c.callback), self.cfg
                        ).string
            ...
            try:
                if text:
                    c.callback.edit_message_text(
                        text, reply_markup=markup or self.gen_markup(LANG, m)
                    )
                elif markup:
    ...
    def run(self, LANG, bot, m):
        ...
        if sender_is_admin(m):
            if len(args) > 1:
                        ...
                        if option:
                            ...
                            if option.is_valid(value):
                                if self.usr.set(m.chat.id, item, option.type(value)):
                                    m.reply_text(
                                        LANG("SETTINGS_SET_TO").format(
                                            item,
                                            self.usr.get(m.chat.id, item),
                                            old_value,
                                        )
                                    )
                                else:
                                    m.reply_text(
                                        LANG("SETTINGS_COULD_NOT_SET").format(
                                            item,
                                            self.usr.get(m.chat.id, item),
                                            old_value,
                                        )
                                ...
                                m.reply_text(
                                    LANG("SETTINGS_NOT_VALID_VALUE_FOR").format(
                                        html.escape(value), item
                                    )
                                )
                        else:
                            m.reply_text(
                                LANG("NOT_RECOGNIZED").format(
                ...
                bot.send_message(
                    m.chat.id,
                    LANG("SETTINGS_FOR_THIS_CHAT"),
                    reply_markup=self.gen_markup(LANG, m),
                )
        else:
