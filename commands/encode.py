...
class CmdEncode(BaseCommand):
    ...
    def function(self, LANG, args, alternate_text: str) -> (str, str):
        out, ok = "", False
        if (len(args) < 4 and alternate_text == "") or (
            len(args) < 3 and alternate_text != ""
        ):
            ...
            if dec and enc:
                if dec == enc:
                    ...
                    try:
                        # decode from
                        if dec == "binary":
                            out = self.binary2text(out)
                        elif dec == "base64":
                            out = self.base642text(" ".join(out))
                        elif dec == "text":
                            out = " ".join(out)
                        # encode to
                        if enc == "binary":
    ...
    def run(self, LANG, bot, m):
        text, ok = self.function(
            LANG,
            (m.text or m.caption).split(" "),
            ""
            if m.reply_to_message is None
            else (m.reply_to_message.text or m.reply_to_message.caption).split(" "),
        )
            ...
            return [
                InlineQueryResultArticle(
                    ...
                    input_message_content=InputTextMessageContent(
                        LANG("PROVIDE_DECODING_ENCODING_TEXT")
                    ),
