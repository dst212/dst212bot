...
class CmdPoGo(BaseCommand):
    ...
    def rank_buttons(self, rank, min_iv):
        ...
        ranks = [rank - 10, rank - 1, rank + 1, rank + 10]
        i = 0
        while i < len(ranks):
            if ranks[i] < 1:
                ranks[i] += max_rank
            elif ranks[i] > max_rank:
                ranks[i] %= max_rank
            i += 1
        return ranks
    ...
    def markup(self, c, original=None):
        if c:
            if original is None:
                original = c[1]
            callback = (
                f"{self.name} {c[0]}" + " {} " + " ".join([str(i) for i in c[2:]])
            )
    ...
    def callback(self, LANG, bot, c):
        ...
        original = c.callback.message.reply_markup.inline_keyboard[0][
            0
        ].callback_data.split(" ")[-4]
        try:
            c.callback.edit_message_text(
                f"<b>{title}</b>\n\n" + out if title else out,
                reply_markup=self.markup(config, original),
            )
