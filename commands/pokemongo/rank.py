    ...
    for a_iv in range(min_iv, 16):
        for d_iv in range(min_iv, 16):
            for h_iv in range(min_iv, 16):
                ...
                h = floor(pkm["hp"]) + h_iv
                lvl = level(a, d, h, max_cp, max_lvl)
                p = product(lvl, a, d, h)
                products += [
                    [
                        p,
...
def pokemon_output(
    ...
    out += (
        ...
        + "<i>Attack</i> : <code>{}</code>\n".format(
            round((pkm["attack"] + v[1]) * cpm * 100) / 100
        )
        + "<i>Defense</i> : <code>{}</code>\n".format(
            round((pkm["defense"] + v[2]) * cpm * 100) / 100
        )
...
def get_rank():
    ...
    def load(
        self, pkm: dict, min_iv: int, max_cp: int, max_lvl: int
    ) -> list[list[int]]:
    ...
    def f(self, LANG, args: list[str]) -> (str, str, list):
        ...
        while len(args) > 1 and not re.sub(r"[\/\.]", "", args[1]).isnumeric():
        ...
        if len(args) < 2:
            ...
            try:
                ...
                iv = [int(i) for i in re.split(r"[\/\.]", args[1])]
                # IVs are invalid
        ...
        for arg in args[2:]:
            ...
            # parsing
                ...
                try:
                    ...
                    return (
                        LANG("ERROR"),
                        LANG("POGO_NOT_RECOGNIZED").format(html.escape(arg)),
                        None,
                    )
            count += 1
        if max_cp < 10:
        ...
        if iv is not None:
            ...
            for v in p:
                if v[1] == iv[0] and v[2] == iv[1] and v[3] == iv[2]:
                    title, tmp = pokemon_output(
                        pkm, v, p[0][0], rank, max_cp, min_iv, max_lvl, i=i
                    )
                    rank = i
                    break
                i += 1
            ...
            if not 0 <= rank < max_rank:
                return (
                    LANG("ERROR"),
                    LANG("POGO_RANK_MUST_BE_BETWEEN").format(max_rank),
                    None,
                )
            title, tmp = pokemon_output(
                pkm, p[rank], p[0][0], rank, max_cp, min_iv, max_lvl
            )
