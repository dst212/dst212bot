...
from pyrogram.types import User, Chat, CallbackQuery
...
def can_delete(m) -> bool:
    return (
        ...
        or m.chat.get_member("me").privileges
        and m.chat.get_member("me").privileges.can_delete_messages
    )
...
def command_entry(LANG, cmd: BaseCommand, entry=None, inline_notice=True):
    return (
        ...
        if cmd
        else LANG("NO_ENTRY_FOR").format(html.escape(entry))
        if entry
        else LANG("NO_ENTRY")
    )
